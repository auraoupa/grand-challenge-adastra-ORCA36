         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1
               ...
               IF(  (misfdep(ji,jj) > 1) .AND. (mbathy(ji,jj) > 0) ) THEN
                  ...
                  ibathy=ibathyij; ishl=ji; jshl=jj
                  DO imin = -1,1
                     IF (mbathy(ji+imin,jj) < ibathy .AND. mbathy(ji+imin,jj) >= misfdep(ji,jj) ) THEN
                        ibathy=mbathy(ji+imin,jj); ishl=ji+imin; jshl=jj
                     END IF
                  END DO
                  DO jmin = -1,1
                     IF (mbathy(ji,jj+jmin) < ibathy .AND. mbathy(ji,jj+jmin) >= misfdep(ji,jj) ) THEN
                        ibathy=mbathy(ji,jj+jmin); ishl=ji; jshl=jj+jmin
                     END IF
                  END DO
                  ...
                  zhw = bathy(ishl,jshl) - risfdep(ji,jj)
                  IF ( zhw < rn_isfhw_min ) THEN
                     risfdep(ji,jj) = bathy(ishl,jshl) - rn_isfhw_min
                     DO jk = 2, jpkm1
                        zdepth = gdepw_1d(jk+1) - MIN( e3zps_min, e3t_1d(jk)*e3zps_rat )
                        IF( risfdep(ji,jj) > 0._wp .AND. risfdep(ji,jj) >= zdepth ) imisfdep(ji,jj) = jk+1
                     END DO
                  END IF
